...
import { idParams } from "@/components/pages/Project/ProjectPage";
import { useMemo, memo, useCallback } from "react";
import { useScrollScale } from "@/components/useScrollScale";

const ProjectCard = memo(({ project, isMobile, router }: { project: typeof ProjectsData[0]; isMobile: boolean | undefined; router: any }) => {
    ...
    <Box w={'85vw'} mt={100} mb={60}>
      ...
      <Grid gutter={isMobile ? 16 : 24}>
        {otherProjects.map((project) => (
          <ProjectCard key={project.id} project={project} isMobile={isMobile} router={router} />
        ))}
      </Grid>
    </Box>
  );
